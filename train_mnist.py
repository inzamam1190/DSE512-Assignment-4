...
def train(num_epochs=30, batch_size=64, learning_rate=0.001):
    ...
    for ep in tqdm(epbar):
        ...
        if rank == 0:
            ...
            print("epoch", ep, "num_iters", num_iters, "loss", ep_loss, "accuracy",accuracy,"elapsed time (s)", 
                  perf_counter() - start)
        epoch_losses.append(ep_loss)
        elapsed_times.append(perf_counter()  - start)
        accuracies.append(accuracy)
        
        metrics = pd.DataFrame({'epoch_accuracy': accuracies, 'epoch_losses': epoch_losses, 'elapsed_time':
        elapsed_times})
        metrics.to_csv(f'metrics_{world_size}.csv')
